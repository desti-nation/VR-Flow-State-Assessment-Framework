...
def save_logs(output_directory, hist, y_pred, y_pred_probabilities, y_true, duration, lr=True, y_true_val=None,
    ...
    hist_df.to_csv(path_join(output_directory , 'history.csv'), index=False)

    df_metrics = calculate_metrics(y_true, y_pred, y_pred_probabilities, duration, y_true_val, y_pred_val)
    df_metrics.to_csv(path_join(output_directory , 'df_metrics.csv'), index=False)
    ...
    df_best_model.to_csv(path_join(output_directory , 'df_best_model.csv'), index=False)
    print('save log',path_join(output_directory , 'df_best_model.csv'))

    # plot losses
    plot_epochs_metric(hist, path_join(output_directory , 'epochs_loss.png'))
    plot_predictions(y_pred, y_true, path_join(output_directory , 'predictions.png'))
    save_predictions(y_true, y_pred, y_pred_probabilities, path_join(output_directory ,"predictions.txt"))
