...
import matplotlib
import numpy as np
from matplotlib import pyplot, image, pyplot as plt
...
# # # # #
# FUNCTIONS
def draw_blink_tomato(blinks,savefilename=None):
	blink_starts = [blink[0] for blink in blinks]
	blink_ends = [blink[1] for blink in blinks]
	blink_durations = [blink[2] for blink in blinks]
	# 绘制眨眼时间序列
	...
	plt.title('Blink Time Series')
	plt.grid(True)
	plt.show()
	# 绘制眨眼持续时间的柱状图
	plt.figure(figsize=(6, 4))
	plt.bar(range(len(blink_durations)), blink_durations, color='skyblue')
	plt.xlabel('Blink Index')
	plt.ylabel('Duration (seconds)')
	plt.title('Blink Duration')
	plt.xticks(range(len(blink_durations)), [str(i) for i in range(1, len(blink_durations) + 1)])
	plt.grid(axis='y')
	plt.show()
