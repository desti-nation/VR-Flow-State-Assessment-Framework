...
def blink_detection(x, y, time, missing=0.0, minlen=10):
	...
	return Sblk, Eblk
def blink_detection_tomato(leftopenness,rightopenness,time):
	"""
	custom define
	:param leftopenness:
	:param rightopenness:
	...
	for i in range(len(time)):
		curOpen = (leftopenness[i] == 1) and (rightopenness[i] == 1)
		if open is not None and open != curOpen:
			if startTime == -1 : # 眨眼开始
				startTime = time[i]
			else: # 眨眼结束
				duration = time[i] - startTime
				if duration < 5000:
					Eblk.append([startTime,time[i],duration])
				startTime = -1
		if open is None :
			if curOpen:
				open = curOpen
